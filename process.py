...
def prompt_handler(step, speciality=None, job=None, types=None, plan=None, data=None):
    if step == 'first':
        return f'我是{speciality}专业的实习生，根据我的专业和我的工作{job}，写一份实习计划，要求：150字左右。'
    if step == 'second':
        return f'我将给你一份实习计划：{plan}，根据此计划写一份实习{types}，要求150字左右，包含项目名字（不包含我的专业名字），项目记录，项目总结，并以json格式输出。'
    if step == 'third':
